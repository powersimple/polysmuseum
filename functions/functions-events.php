...
function displayProfileIndex($profile_sort){
    ...
    usort($profile_sort, function ($a, $b) {
        return $a['sort'] <=> $b['sort']; // Spaceship operator returns -1, 0, or 1
    });
      ...
      if($child['classes'][0] == 'nomination' || $child['classes'][0] == 'honor'){     }
        ...
        if($counter == 0){
          ...
          print "</td>";
            ...
            print "</td>";
           print  "<tr class='session-detail'><td colspan='3'>";
...
function ros_items($session, $event_title = ''){
    ...
    foreach($session['children'] as $p => $speaker){ // THIS IS THE SPEAKER LOOP
        ...
        $guest_type = @$speaker['guest_type'];
         ...
         $appearance_type = @$speaker['appearance_type'];
         if($guest_type ==  ''){
         //  PRINT $guest_type = "<STRONG>GUEST TYPE NOT SET</STRONG>";                
         }
        ...
        $speaker_name = $speaker['post']->post_title;
         ...
         $speaker_status[$session['post']->ID]['speakers'][$speaker['post']->ID] = [
             ...
             $this_status['appearance_type'] = $appearance_type;
            ...
            if(!is_array(@$invitation_statuses["$confirmation_status"])){
             $invitation_statuses["$confirmation_status"] = [];
         }
        ...
        // var_dump($send_calendar, $is_profile);
         ...
         //    print "<span class='confirmation-notes'>".ucfirst(@$point_of_contact)." $notes";
             //var_dump($meta);
             print "</span></li>";
 //get_nominees_and_winners($speaker['children'],0);//recursive nominees loop

//recursive call  ros_items($speaker['children']);
if(array_key_exists("children",$speaker)){
  
    if(is_array($speaker['children'])){
        rosSessionChildren($session['children']);
...
function rosSessionChildren($children){
       ...
       //     print_r(@$children);
            foreach($children as $c => $child){
                extract($child);
               // get_nominees_and_winners($child['children'],0);
               //print $title;
            // print "<li class='".@$confirmation_status."'>$title </li>";
...
function getFieldNames($data, $prefix = '') {
    $fields = [];
    ...
    if (is_array($data) || is_object($data)) {
        foreach ($data as $key => $value) {
            ...
            // Recursively get nested field names
            if (is_array($value) || is_object($value)) {
                $fields = array_merge($fields, getFieldNames($value, $fullKey));
            }
        }
    }
